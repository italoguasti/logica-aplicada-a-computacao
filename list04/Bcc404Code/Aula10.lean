...
declare_syntax_cat expr_lit
declare_syntax_cat expr_stmt

syntax num : expr_lit
syntax ident : expr_stmt
syntax expr_lit : expr_stmt
syntax "(" expr_stmt ")" : expr_stmt
syntax expr_stmt "+" expr_stmt : expr_stmt

partial def elabExpression : Syntax → TermElabM Expr
| `(expr_stmt| $x:ident) =>
  mkAppM ``Expression.Variable #[mkStrLit x.getId.toString]
| `(expr_stmt| $n:num) =>
  mkAppM ``Expression.Constant #[mkNatLit n.getNat]
| `(expr_stmt| ($x:expr_stmt)) =>
  elabExpression x
| `(expr_stmt| $x:expr_stmt + $y:expr_stmt) => do
  let x' ← elabExpression x
  let y' ← elabExpression y
  mkAppM ``Expression.Sum #[x', y']
| _ => throwUnsupportedSyntax

declare_syntax_cat stmt

syntax ident ":=" expr_stmt : stmt
syntax "print" expr_stmt : stmt
syntax stmt ";" stmt : stmt
syntax "do_nothing" : stmt

partial def elabStatement : Syntax → TermElabM Expr
| `(stmt| $x:ident := $e:expr_stmt) => do
  let e' ← elabExpression e
  let varExpr := mkStrLit x.getId.toString
  mkAppM ``Statement.Assignment #[varExpr, e']
| `(stmt| print $e:expr_stmt) => do
  let e' ← elabExpression e
  mkAppM ``Statement.Print #[e']
| `(stmt| $s1:stmt ; $s2:stmt) => do
  let s1' ← elabStatement s1
  let s2' ← elabStatement s2
  mkAppM ``Statement.Sequence #[s1', s2']
| `(stmt| do_nothing) =>
  return mkConst ``Statement.DoNothing
| _ => throwUnsupportedSyntax

elab "begin" p:stmt "end" : term => elabStatement p
